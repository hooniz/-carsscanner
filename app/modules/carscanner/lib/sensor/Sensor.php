...
namespace App\Modules\CarScanner\Sensor;
...
class Sensor
{
	...
	public function getMessagesByStatistic(array $statistic): array
	{
		...
		if($statistic)
		{
			$defaultMessages = require_once $_SERVER['DOCUMENT_ROOT'] . "/app/modules/carscanner/include/messages.php";
			$wrongData = [];
			foreach ($statistic as $statValue)
			{
				if($statValue['error'])
				{
					$wrongData[] = $statValue['code'];
				}
			}
