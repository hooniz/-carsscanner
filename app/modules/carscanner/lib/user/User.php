...
namespace App\Modules\CarScanner\User;
...
class User
{
    static public function getUserById(int $id)
    {
        $user = (new SelectQuery())
            ->setTableName('users')
            ->setSelect(['email', 'code', 'model', 'manufacturedYear', 'role', 'sensor', 'engineType', 'fuelType'])
            ->setWhere([
    ...
    static public function editProfile(Request $request): array
    {
        ...
        if(!$errors)
        {
            ...
            if(!$userUpdate->getError())
            {
                ...
                $sql = "UPDATE `cars` SET `manufacturedYear` = :manufacturedYear, `model` = :model, `fuelType` = :fuel WHERE `id` = :id";
                $carUpdate = $db->query($sql, [
                    'manufacturedYear' => $userData['datepicker'],
                    'model' => $userData['car'],
                    'fuel' => $userData['fuel_type']??92,
                    'id' => $userCar
                ]);
            }else
            {
